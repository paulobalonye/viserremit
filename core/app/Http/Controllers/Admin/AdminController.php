...
class AdminController extends Controller
{
    public function dashboard()
    {
        ...
        $sendMoneyData = SendMoney::selectRaw('sending_country.id as sending_country_id, sending_country.currency as sending_currency, sending_country.name as sending_country, sending_country.image as sending_country_image, recipient_country.id as recipient_country_id, recipient_country.name as recipient_country, recipient_country.image as recipient_country_image, SUM(send_money.sending_amount) as total_amount, SUM(send_money.base_currency_amount) as total_base_amount')
            ...
            ->where('send_money.status', Status::SEND_MONEY_COMPLETED)
        ...
        $selectedSendingCurrency = $sendingCountries->first()->currency ?? null;
        $selectedReceivingCurrency = $receivingCountries->where('currency', '!=', $selectedSendingCurrency)->first()->currency ?? null;
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'withdrawals', 'payment', 'sendMoney', 'sendingCountries', 'receivingCountries', 'sendMoneyStatistics', 'sendMoneyData', 'selectedSendingCurrency', 'selectedReceivingCurrency'));
    }

    public function sendMoneyStatistics(Request $request)
    {
        $format = '%M-%Y';
        $dates = $this->getAllMonths(now()->subYear(), now());


        $sendMoney = SendMoney::where('created_at', '>=', now()->subYear())
            ->completed()
            ->where('sending_currency', $request->sending_currency)
            ->where('recipient_currency', $request->recipient_currency)
            ->selectRaw('SUM(sending_amount) AS sending_amount')
            ->selectRaw('SUM(recipient_amount) AS recipient_amount')
            ->selectRaw('SUM(base_currency_amount) AS base_currency_amount')
        ...
        foreach ($dates as $date) {
            $data[] = [
                'created_on' => $date,
                'sending_amount' => getAmount($sendMoney->where('created_on', $date)->first()?->sending_amount ?? 0),
                'recipient_amount' => getAmount($sendMoney->where('created_on', $date)->first()?->recipient_amount ?? 0),
                'base_currency_amount' => getAmount($sendMoney->where('created_on', $date)->first()?->base_currency_amount ?? 0)
            ];
        }
        ...
        $report['base_currency_amount']   = $data->pluck('base_currency_amount');
        $report['data']     = [
            [
                'name' => trans('Sending Amount'),
                'data' => $data->pluck('sending_amount'),
            ],
            [
                'name' => trans('Recipient Amount'),
                'data' => $data->pluck('recipient_amount'),
            ]
        ];
    ...
    public function depositAndWithdrawReport(Request $request)
    {
        ...
        $deposits = Deposit::whereNot('agent_id', 0)
            ->successful()
        ...
        $withdrawals = Withdrawal::whereNot('agent_id', 0)
            ->approved()
            ->whereDate('created_at', '>=', $request->start_date)
            ->whereDate('created_at', '<=', $request->end_date)
            ->selectRaw('SUM(amount) AS amount')
