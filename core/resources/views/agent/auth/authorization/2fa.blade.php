        ...
        <div class="container custom-container d-flex justify-content-center">
            <div class="login-area">
                <div class="text-center mb-3">
                    <h2 class="text-white mb-2">@lang('Account authorization')</h2>
                    <p class="text-white mb-2">@lang('2FA Verification')</p>
                    @if ($errors->has('resend'))
                        <small class="text-center text-danger d-block">{{ $errors->first('resend') }}</small>
                    @endif
                </div>
                <form action="{{ route('agent.go2fa.verify') }}" method="POST" class="login-form w-100 disableSubmission">
                    @csrf
